...
def response_function (mark,model,year_from,year_to,fuel,gear_type,owners_count_group,page):
    body = {"year_from":year_from,
        ...
        #"displacement_to":6000,
         ...
         "owners_count_group":[owners_count_group],
    ...
    response = fetch("https://auto.ru/-/ajax/desktop/listing/", {
                    "headers": {
                                ...
                                "x-retpath-y": webapp.config.REFERER,
                                "cookie": webapp.config.COOKIE,
                                "Referer": webapp.config.REFERER,
...
def create_csv(mark,model,year_from,year_to,fuel,gear_type,owners_count_group):
    auto_data = response_function(mark,model,year_from,year_to,fuel,gear_type,owners_count_group,0)
    #print(auto_data)
    ...
    for list in range(total_lists+1):
        auto_data = response_function(mark,model,year_from,year_to,fuel,gear_type,owners_count_group,list)
...
def plot(mark,model,year_from,year_to,fuel,gear_type,owners_count_group):

    
    create_csv(mark,model,year_from,year_to,fuel,gear_type,owners_count_group)
    ...
    link_plot= f'../static/images/{mark}_{model}.jpg'
    return link_plot
